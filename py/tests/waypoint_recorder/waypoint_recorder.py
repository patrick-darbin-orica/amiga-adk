...
- On every Enter key press, appends a CSV row with:
  ...
  where dy = pose.translation[0], dx = pose.translation[1] (both rounded to 3f)
...
Usage:
  python waypoint_recorder.py --csv waypointTest1.csv
"""
...
class PoseSaver:
    ...
    def save_current(self) -> bool:
        """Append a CSV row using the latest dx/dy (rounded to 3f). Returns True if saved."""
        ...
        row = [
            ...
            f"{-self.latest_dx:.3f}", #flipped for NED convention
            f"{self.latest_dy:.3f}",
        ]
